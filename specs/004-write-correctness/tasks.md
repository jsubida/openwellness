...
- [x] 1. Add the real-Sync-Gateway harness: Couchbase Server 6.6 CE + Sync Gateway 2.8.2 services in
...
- [x] 2. Make `channels` and `_rev` first-class on the domain `BaseEntity`, and update the class
...
- [x] 3. Add `ChannelsInvariantError` and `RevisionConflictError`, then the fail-closed guard on the
...
- [x] 4. Stamp the supplied actor in the driver instead of the `"scheduler"` literal, and raise
...
- [x] 5. Add `application/actors.py` and thread a required `actor` parameter through the repository
...
- [x] 6. Pass the authenticated acting principal at every API write call site. _(Story 4)_
...
- [x] 7. Add the real-Sync-Gateway round-trip integration suite with the second-subscriber
...
- [x] 8. Add the guard-rejection re-fetch comparison and the stale-revision conflict case to the
