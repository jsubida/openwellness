...
class CBCPAPSessionRepository(
    ...
    def _build_get_between_query(
        ...
        q = (
            ...
            f"WHERE type = $type "
            f"AND owner = $owner "
            f"AND dateOfSleep BETWEEN $start AND $end "
            f"ORDER BY dateOfSleep, createdAt DESC"
        )
        params = {
            "type": CBCPAPSession.type,
            "owner": owner_id,
    ...
    def _build_get_query(
        ...
        q = (
            ...
            f"WHERE type = $type "
            f"AND owner = $owner "
            f"AND dateOfSleep = $dateOfSleep "
            f"ORDER BY dateOfSleep, createdAt DESC"
        )
        params = {
            "type": CBCPAPSession.type,
            "owner": owner_id,
            "dateOfSleep": arg.format("YYYY-MM-DD"),
        }
        return q, params
